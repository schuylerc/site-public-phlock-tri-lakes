...
$co = (Get-ChildItem -Path (Join-Path $basePath 'Community Outreach') -File | Sort-Object @{Expression={ if ($_.Name -match '^\d+') { [double]$matches[0] } else { 0 } }}, Name).Name
$ph2026 = (Get-ChildItem -Path (Join-Path $basePath '2026 Phlock Photos') -File | Sort-Object @{Expression={ if ($_.Name -match '^\d+') { [double]$matches[0] } else { 0 } }}, Name).Name
$ph2025 = (Get-ChildItem -Path (Join-Path $basePath '2025 Phlock Photos') -File | Sort-Object @{Expression={ if ($_.Name -match '^\d+') { [double]$matches[0] } else { 0 } }}, Name).Name
...
Write-Host "Replaced arrays. New 2026 length: $($ph2026.Count)"
